...
Процедура ОбработатьСообщения() Экспорт
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	ВходящиеСообщения.ДатаФормированияВмилисекундах";
...
Процедура ЗаписатьВходящееСообщение(ОтветноеСообщение, ИмяОчереди)
	...
	ВходящееСообщение.ДатаФормированияВмилисекундах = ТекущаяУниверсальнаяДатаВМиллисекундах();
...
Процедура ОбработатьОбъектВбазе(ТелоСообщения)
	...
	ИначеЕсли Объект.ПолноеИмяОбъектаМетаданных = "Документ.ЗаказКлиента" Тогда
		Документы.ЗаказНаСборку.ЗагрузитьДокумент(Объект);	
	КонецЕсли;
